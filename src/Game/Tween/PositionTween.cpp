...
namespace MinerSpeed
{
    ...
    PositionTween::PositionTween(King::Engine &engine, const King::Engine::Texture &texture, const glm::vec2 &startPos,
        ...
        : mEngine(&(engine)), mTexture(texture), mStartPos(startPos), mEndPos(endPos), mDuration(duration), mAcceleration(acceleration), mTime(0.0f), mCurrentPos(0.0f, 0.0f), mDirectionMask(0.0f, 0.0f), mIsCompleted(false)
    {
    }
