...
namespace King
{
        ...
        : mPimpl(new EngineImplementation)
    {
        ...
        std::vector<std::string> assetfiles{ "/arts/BackGround.jpg",
                                             "/arts/Blue.png", "/arts/BlueSelected.png", "/arts/Green.png", "/arts/GreenSelected.png",
                                             "/arts/Purple.png", "/arts/PurpleSelected.png", "/arts/Red.png", "/arts/RedSelected.png",
                                             "/arts/Yellow.png", "/arts/YellowSelected.png" };

        std::string font = assets;
        font += "/font/berlin_sans_demi_72_0.png";

        std::vector<int> assetname{ Engine::TEXTURE_BACKGROUND,
                                    Engine::TEXTURE_BLUE, Engine::TEXTURE_BLUE_SELECTED, Engine::TEXTURE_GREEN, Engine::TEXTURE_GREEN_SELECTED,
                                    Engine::TEXTURE_PURPLE, Engine::TEXTURE_PURPLE_SELECTED, Engine::TEXTURE_RED, Engine::TEXTURE_RED_SELECTED,
                                    Engine::TEXTURE_YELLOW, Engine::TEXTURE_YELLOW_SELECTED};

        for (size_t i = 0; i < assetfiles.size(); i++) {
            auto texture = assets;
            texture += assetfiles[i];
            mPimpl->mSdlSurfaceContainer[assetname[i]].reset(new SdlSurface(texture.c_str()));
        }
    ...
    void Engine::Render(Engine::Texture texture, const glm::mat4 &transform)
    {
        ...
        glBegin(GL_QUADS);
        glTexCoord2i(0, 1);
        glVertex2i(0, surface.Height());
        glTexCoord2i(1, 1);
        glVertex2i(surface.Width(), surface.Height());
        glTexCoord2i(1, 0);
        glVertex2i(surface.Width(), 0);
        glTexCoord2i(0, 0);
        glVertex2i(0, 0);
        glEnd();
    }
    ...
    void Engine::Write(const char *text, const glm::mat4 &transform)
    {
        ...
        for (; *text; ++text) {
            ...
            glBegin(GL_QUADS);
            glTexCoord2f(uvLeft / 2.0f, uvTop / 2.0f);
            glVertex2f(worldLeft * TextScale, worldTop * TextScale);
            glTexCoord2f(uvRight / 2.0f, uvTop / 2.0f);
            glVertex2f(worldRight * TextScale, worldTop * TextScale);
            glTexCoord2f(uvRight / 2.0f, uvBottom / 2.0f);
            glVertex2f(worldRight * TextScale, worldBottom * TextScale);
            glTexCoord2f(uvLeft / 2.0f, uvBottom / 2.0f);
            glVertex2f(worldLeft * TextScale, worldBottom * TextScale);
            glEnd();
            advance += g.advance;
        }
    }
